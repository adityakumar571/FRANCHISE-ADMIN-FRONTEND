...
import { TrendingDown, Filter, Printer } from 'lucide-react'
...
const ExpenseRegister = () => {
  ...
  const buildFilterParams = (filters) => {
    const q = new URLSearchParams({ voucherType: 'Expense', status: 'Active' })
  ...
  React.useEffect(() => {
    getRequest('hr/account-heads?accountType=Expense&limit=200')
  ...
  React.useEffect(() => {
    ...
    getRequest(`hr/vouchers?${q.toString()}`)
      ...
      .catch(() => toast.error('Failed to load expense register'))
    ...
    <div className="bg-slate-50 min-h-screen p-4 md:p-6 space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-black text-slate-800 flex items-center gap-2">
            <TrendingDown className="w-5 h-5" style={{ color: COLOR }} /> Expense Register
          </h1>
          <p className="text-xs text-slate-400 mt-0.5 font-medium">All expense vouchers — month-wise or date-range</p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          {applied && (
            <button onClick={handleClear} className="text-sm font-bold self-center text-slate-400 hover:text-slate-600 transition">Clear</button>
          )}
        </div>
      </div>
      ...
      {applied && summary && !summaryLoading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Total Expense</p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="space-y-1.5 max-h-28 overflow-y-auto">
              {Object.entries(summary.byHead).sort(([,a],[,b])=>b-a).map(([head, amt]) => (
      ...
      {!applied ? (
        <div className="bg-white border border-slate-200 rounded-2xl py-16 text-center text-slate-400 text-sm shadow-sm font-medium">
          Select filters and click <b className="text-slate-600">Apply</b> to view the expense register
        </div>
      ) : loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl py-10 text-center shadow-sm">
          <Empty description="No expense records found" />
        </div>
      ) : (
              ...
              {data.map((v, idx) => (
                <tr key={v._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="px-2 py-0.5 rounded-lg text-xs font-bold" style={{ background: `${COLOR}15`, color: COLOR }}>{v.paymentMode}</span>
                  </td>
