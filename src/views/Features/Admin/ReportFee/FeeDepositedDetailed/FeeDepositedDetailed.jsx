...
const FeeDepositedDetailed = () => {
  ...
  const [draftFilters, setDraftFilters] = useState({ sessionId: null, classId: null, sectionId: null, fromDate: null, toDate: null })
  const [appliedFilters, setAppliedFilters] = useState({ sessionId: null, classId: null, sectionId: null, fromDate: null, toDate: null })

  useEffect(() => { if (!currentSession?._id) return; const b = { sessionId: currentSession._id, classId: null, sectionId: null, fromDate: null, toDate: null }; setDraftFilters(b); setAppliedFilters(b) }, [currentSession])
  ...
  const fetchReport = async (filters, pageNo = 1, pageSize = 10) => {
    try {
      setLoading(true)
      const params = { sessionId: filters.sessionId, classId: filters.classId, sectionId: filters.sectionId, fromDate: filters.fromDate, toDate: filters.toDate, page: pageNo, limit: pageSize }
      Object.keys(params).forEach((k) => params[k] == null && delete params[k])
      const res = await getRequest(`reports/fee-deposited-detailed?${new URLSearchParams(params)}`)
  ...
  const handleClear = () => { const r = { sessionId: currentSession?._id || null, classId: null, sectionId: null, fromDate: null, toDate: null }; setIsApplied(false); setDraftFilters(r); setAppliedFilters(r); setPage(1) }

  return (
    <div className="min-h-screen">
      <div className="bg-white border rounded-lg px-4 py-3 mb-4">
        <h1 className="text-lg font-semibold flex items-center gap-2"><IndianRupee className="text-red-500" /> Fee Deposited Statement (Detailed)</h1>
        <p className="text-sm text-gray-500">Receipt-wise detailed fee deposited statement</p>
      </div>
...
export default FeeDepositedDetailed
