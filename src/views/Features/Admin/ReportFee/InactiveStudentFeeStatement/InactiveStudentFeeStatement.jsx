...
const InactiveStudentFeeStatement = () => {
  ...
  const fetchReport = async (filters, pageNo = 1, pageSize = 10) => {
    try {
      ...
      const res = await getRequest(`reports/inactive-student-fee-statement?${new URLSearchParams(params)}`)
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-white border rounded-lg px-4 py-3 mb-4">
        <h1 className="text-lg font-semibold flex items-center gap-2"><IndianRupee className="text-red-500" /> Fee Statement (In-Active Student)</h1>
        <p className="text-sm text-gray-500">Fee statement for inactive / TC students</p>
      </div>
...
export default InactiveStudentFeeStatement
