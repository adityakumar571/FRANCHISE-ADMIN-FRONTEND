...
const FeeRegisterDetailed = () => {
  ...
  const [draftFilters, setDraftFilters] = useState({ sessionId: null, classId: null, sectionId: null, fromDate: null, toDate: null, studentSearch: null, receiptNo: null })
  const [appliedFilters, setAppliedFilters] = useState({ sessionId: null, classId: null, sectionId: null, fromDate: null, toDate: null, studentSearch: null, receiptNo: null })

  useEffect(() => { if (!currentSession?._id) return; const b = { sessionId: currentSession._id, classId: null, sectionId: null, fromDate: null, toDate: null, studentSearch: null, receiptNo: null }; setDraftFilters(b); setAppliedFilters(b) }, [currentSession])
  ...
  const fetchReport = async (filters, pageNo = 1, pageSize = 10) => {
    try {
      setLoading(true)
      const params = { sessionId: filters.sessionId, classId: filters.classId, sectionId: filters.sectionId, fromDate: filters.fromDate, toDate: filters.toDate, studentSearch: filters.studentSearch, receiptNo: filters.receiptNo, page: pageNo, limit: pageSize }
      Object.keys(params).forEach((k) => params[k] == null && delete params[k])
      const res = await getRequest(`reports/fee-register-detailed?${new URLSearchParams(params)}`)
  ...
  const handleClear = () => { const r = { sessionId: currentSession?._id || null, classId: null, sectionId: null, fromDate: null, toDate: null, studentSearch: null, receiptNo: null }; setIsApplied(false); setSections([]); setDraftFilters(r); setAppliedFilters(r); setPage(1) }

  return (
    <div className="min-h-screen">
      <div className="bg-white border rounded-lg px-4 py-3 mb-4">
        <h1 className="text-lg font-semibold flex items-center gap-2"><IndianRupee className="text-red-500" /> Fee Register (Detailed)</h1>
        <p className="text-sm text-gray-500">Detailed fee register with transaction-wise breakdown</p>
      </div>
      <div className="bg-white rounded border p-4 mb-4">
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex flex-col"><label className="font-medium text-gray-600 mb-1">Class</label><Select allowClear placeholder="Select Class" value={draftFilters.classId} className="w-[200px]" onChange={(v) => setDraftFilters((p) => ({ ...p, classId: v || null, sectionId: null }))}>{classes.map((c) => <Option key={c._id} value={c._id}>{c.name}</Option>)}</Select></div>
          <div className="flex flex-col"><label className="font-medium text-gray-600 mb-1">Section</label><Select allowClear placeholder="Select Section" value={draftFilters.sectionId} className="w-[160px]" disabled={!draftFilters.classId} onChange={(v) => setDraftFilters((p) => ({ ...p, sectionId: v || null }))}>{sections.map((s) => <Option key={s._id} value={s._id}>{s.name}</Option>)}</Select></div>
          ...
          <div className="flex flex-col"><label className="font-medium text-gray-600 mb-1">Student (ID / Name)</label><Input allowClear placeholder="Search student..." value={draftFilters.studentSearch || ''} className="w-[180px]" onChange={(e) => setDraftFilters((p) => ({ ...p, studentSearch: e.target.value || null }))} /></div>
          <div className="flex flex-col"><label className="font-medium text-gray-600 mb-1">Receipt No</label><Input allowClear placeholder="e.g. RCPT-123" value={draftFilters.receiptNo || ''} className="w-[180px]" onChange={(e) => setDraftFilters((p) => ({ ...p, receiptNo: e.target.value || null }))} /></div>
            ...
            {!loading && data.length === 0 ? <tr><td colSpan="9" className="text-center py-6"><Empty description="No Records Found" /></td></tr>
              : data.map((item, i) => (
                <tr key={item._id || i} className="border-b hover:bg-gray-50">
                  ...
                  <td className="text-center text-green-600 font-semibold">₹{item.amount}</td>
...
export default FeeRegisterDetailed
