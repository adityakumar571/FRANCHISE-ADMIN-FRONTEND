...
const NewAdmissionFeeCollection = () => {
  ...
  const [draftFilters, setDraftFilters] = useState({ sessionId: null, classId: null, fromDate: null, toDate: null })
  const [appliedFilters, setAppliedFilters] = useState({ sessionId: null, classId: null, fromDate: null, toDate: null })

  useEffect(() => { if (!currentSession?._id) return; const b = { sessionId: currentSession._id, classId: null, fromDate: null, toDate: null }; setDraftFilters(b); setAppliedFilters(b) }, [currentSession])
  ...
  const fetchReport = async (filters, pageNo = 1, pageSize = 10) => {
    try {
      setLoading(true)
      const params = { sessionId: filters.sessionId, classId: filters.classId, fromDate: filters.fromDate, toDate: filters.toDate, page: pageNo, limit: pageSize }
      Object.keys(params).forEach((k) => params[k] == null && delete params[k])
      const res = await getRequest(`reports/new-admission-fee-collection?${new URLSearchParams(params)}`)
  ...
  const handleClear = () => { const r = { sessionId: currentSession?._id || null, classId: null, fromDate: null, toDate: null }; setIsApplied(false); setDraftFilters(r); setAppliedFilters(r); setPage(1) }

  return (
    <div className="min-h-screen">
      <div className="bg-white border rounded-lg px-4 py-3 mb-4">
        <h1 className="text-lg font-semibold flex items-center gap-2"><IndianRupee className="text-red-500" /> Fee Collection (New Admission Student)</h1>
        <p className="text-sm text-gray-500">Fee collection report for newly admitted students</p>
      </div>
            ...
            {!loading && data.length === 0 ? <tr><td colSpan="10" className="text-center py-6"><Empty description="No Records Found" /></td></tr>
              : data.map((item, i) => (
                <tr key={item._id || i} className="border-b hover:bg-gray-50">
                  ...
                  <td className="text-center">{item.admissionDate || '-'}</td>
                  <td className="text-center font-semibold">₹{item.totalFee}</td>
                  <td className="text-center text-green-600 font-semibold">₹{item.paidAmount}</td>
...
export default NewAdmissionFeeCollection
