...
const SchoolwiseReport = () => {
  ...
  useEffect(() => {
    ...
    getRequest(`examsList?page=1&limit=100&sessionId=${currentSession._id}`)
      .then((res) => {
  ...
  const fetchReport = async (filters) => {
    try {
      ...
      const res = await getRequest(`marks/school-wise-topper?${query}`)
        ...
        <div className="flex justify-between items-center flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold text-[#0c3b73] flex items-center gap-2">
              🏆 School Wise Topper Report
            </h1>

            <p className="text-gray-500 text-sm mt-1">View school toppers and rankings</p>
          </div>

          <ExportButton
            data={visibleData}
            fileName="SchoolWiseTopperReport.xlsx"
            sheetName="School Wise Topper"
          />
        </div>
      </div>
      ...
      {top3.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          {top3.map((item, index) => (
            <div
              key={index}
              className="bg-gradient-to-r from-[#0c3b73] to-[#124d94] text-white rounded-2xl p-3 shadow-lg"
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm opacity-80">Rank {index + 1}</p>
