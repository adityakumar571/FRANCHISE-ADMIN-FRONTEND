...
/**
 * PurchaseDashboard — Real API integration
 */
...
/* ─── Static Alerts (kept as static — will be dynamic later) ─── */
const STATIC_ALERTS = [
  { name: 'Clavm 625 Tablet',     msg: 'Best rate available at Gupta Pharma\n₹2.00 cheaper than your last purchase', btnLabel: 'View Rates',    btnColor: '#16a34a', icon: TrendingDown, iconColor: '#16a34a' },
  { name: 'Azithral 500mg Tablet',msg: 'Stock available with 4 suppliers\nEffective rate: ₹17.20 per strip',         btnLabel: 'View Suppliers',btnColor: '#d97706', icon: Truck,        iconColor: '#d97706' },
  { name: 'Dolo 650 Tablet',      msg: '10+2 scheme available\nBest rate: ₹69.50',                                    btnLabel: 'View Scheme',  btnColor: '#7c3aed', icon: Tag,          iconColor: '#7c3aed' },
  { name: 'Pantop DSR Capsule',   msg: 'Stock running low\nAvailable qty: 25 strips',                                  btnLabel: 'Purchase Now', btnColor: '#dc2626', icon: AlertTriangle,iconColor: '#dc2626' },
  { name: 'Calpol 650 Tablet',    msg: 'Price dropped by ₹1.50\nNew best rate: ₹13.25',                               btnLabel: 'View Details', btnColor: '#0891b2', icon: TrendingDown, iconColor: '#0891b2' },
]

const MODE_CFG   = { Cash: { bg: '#dcfce7', color: '#16a34a' }, UPI: { bg: '#f5f3ff', color: '#7c3aed' }, Credit: { bg: '#fef3c7', color: '#d97706' }, Cheque: { bg: '#e0e7ff', color: '#0c3b73' } }
const STATUS_CFG = { Paid: { bg: '#dcfce7', color: '#16a34a' }, Credit: { bg: '#fef3c7', color: '#d97706' }, Unpaid: { bg: '#fee2e2', color: '#dc2626' }, Due: { bg: '#fef3c7', color: '#d97706' }, Partial: { bg: '#e0f2fe', color: '#0891b2' } }
...
const SkeletonCard = () => (
  <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: '14px', height: 90 }}>
    <div style={{ height: 12, width: '60%', background: '#f3f4f6', borderRadius: 4, marginBottom: 10 }} />
...
export default function PurchaseDashboard() {
  ...
  const fetchDashboard = useCallback(async () => {
    setLoading(true)
    try {
      const res = await getRequest('/franchise/purchase/dashboard')
  ...
  const topItems = dashData?.topItems || []
  const payPie   = dashData?.paymentPie || []
  const hourly   = dashData?.hourlyData  || []
  const suppSumm = dashData?.supplierSummary || {}

  // Client-side filters on invoices
  const filteredInvoices = invoices.filter(inv => {
    const q = search.toLowerCase()
    const matchS = !q || inv.no?.toLowerCase().includes(q) || inv.supplier?.toLowerCase().includes(q)
    const matchP = payFilter === 'All Payment Mode' || inv.mode === payFilter
    const matchSup = suppFilter === 'All Suppliers' || inv.supplier === suppFilter
    const matchQ = quickFilter === 'All' || inv.status === quickFilter ||
      (quickFilter === 'Unpaid' && inv.status !== 'Paid') ||
      (quickFilter === 'Cash'   && inv.mode === 'Cash') ||
      (quickFilter === 'UPI'    && inv.mode === 'UPI') ||
      (quickFilter === 'Credit' && inv.mode === 'Credit')
    return matchS && matchP && matchSup && matchQ
  })
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: '#9ca3af', margin: '3px 0 0' }}>Dashboard › Purchase › Today's Purchases</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => setRefreshKey(k => k + 1)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 14px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10 }}>
        {loading ? Array(6).fill(0).map((_, i) => <SkeletonCard key={i} />) :
          [
            { label: 'Total Purchase Value',  value: fmt(kpi.totalPurchase),  sub: `${kpi.invoiceCount || 0} Invoices`,       color: '#0c3b73', bg: '#e0e7ff', icon: ShoppingCart },
            { label: 'Total Items Purchased', value: kpi.totalItems || 0,     sub: 'Items Purchased',                         color: '#7c3aed', bg: '#f5f3ff', icon: ShoppingBag  },
            { label: 'Total Discount',        value: fmt(kpi.totalDiscount),  sub: `${kpi.totalPurchase > 0 ? ((kpi.totalDiscount/kpi.totalPurchase)*100).toFixed(2) : 0}% of Purchase`, color: '#16a34a', bg: '#dcfce7', icon: Tag },
            { label: 'Total Tax (GST)',        value: fmt(kpi.totalTax),       sub: `${kpi.totalPurchase > 0 ? ((kpi.totalTax/kpi.totalPurchase)*100).toFixed(2) : 0}% of Purchase`,     color: '#d97706', bg: '#fef3c7', icon: IndianRupee },
            { label: 'Net Purchase Value',    value: fmt(kpi.netPurchase),    sub: 'After Discount & Tax',                    color: '#0891b2', bg: '#e0f2fe', icon: TrendingUp   },
            { label: 'Total Paid',            value: fmt(kpi.totalPaid),      sub: `Due: ${fmt((kpi.netPurchase||0)-(kpi.totalPaid||0))}`, color: '#dc2626', bg: '#fee2e2', icon: CheckCircle },
          ].map(k => (
            <div key={k.label} style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: '14px' }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px 260px', gap: 14 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 18px' }}>
          ...
          {loading
            ...
            : <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={hourly} margin={{ top: 16, right: 10, left: 0, bottom: 0 }}>
                <defs>
                ...
                <Area type="monotone" dataKey="v" stroke="#0c3b73" strokeWidth={2.5} fill="url(#purGrad)" dot={{ r: 3, fill: '#0c3b73' }} activeDot={{ r: 5 }} />
              </AreaChart>
            </ResponsiveContainer>
          }
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 18px', display: 'flex', flexDirection: 'column' }}>
          ...
          {loading
            ? <div style={{ height: 150, background: '#f9fafb', borderRadius: 8 }} />
            : <>
              <div style={{ position: 'relative', flex: 1 }}>
                <ResponsiveContainer width="100%" height={150}>
                  <PieChart>
                    <Pie data={payPie.length ? payPie : [{ name: 'No Data', value: 1, color: '#e5e7eb' }]} cx="50%" cy="50%" innerRadius={48} outerRadius={68} dataKey="value" paddingAngle={3} strokeWidth={0}>
                      {(payPie.length ? payPie : [{ color: '#e5e7eb' }]).map((p, i) => <Cell key={i} fill={p.color} />)}
                    </Pie>
                    <Tooltip contentStyle={{ borderRadius: 8, border: 'none', fontSize: 11 }} formatter={v => [fmt(v), '']} />
                  </PieChart>
                </ResponsiveContainer>
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center', pointerEvents: 'none' }}>
                  <p style={{ fontSize: 15, fontWeight: 800, color: '#111827', margin: 0 }}>{fmt(kpi.totalPurchase)}</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
                {payPie.map(p => (
                  <div key={p.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#374151' }}>{fmt(p.value)} <span style={{ color: p.color }}>({p.pct})</span></span>
                  </div>
                ))}
              </div>
            </>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button onClick={() => navigate('/franchise/suppliers')} style={{ fontSize: 11, color: '#0c3b73', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>View All</button>
          </div>
          {loading
            ? Array(5).fill(0).map((_, i) => <div key={i} style={{ height: 14, background: '#f3f4f6', borderRadius: 4, marginBottom: 8 }} />)
            : <>
              {[
                { label: 'Total Suppliers',  value: suppSumm.totalSuppliers || 0 },
                { label: 'New Suppliers',    value: suppSumm.newSuppliers || 0 },
                { label: 'Total Purchase',   value: fmt(suppSumm.totalPurchase) },
                { label: 'Total Discount',   value: fmt(suppSumm.totalDiscount) },
                { label: 'Total Tax (GST)',  value: fmt(suppSumm.totalTax) },
              ].map(s => (
              ...
              <div style={{ borderTop: '2px solid #0c3b73', marginTop: 8, paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 800 }}>
                ...
                <span style={{ color: '#0c3b73' }}>{fmt(suppSumm.netPurchase)}</span>
              </div>
            </>
          }
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#111827', margin: 0 }}>
              Purchase Invoice List <span style={{ fontSize: 11, color: '#9ca3af', fontWeight: 400 }}>({filteredInvoices.length} Invoices)</span>
            </p>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ position: 'relative' }}>
                <Search size={12} style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af' }} />
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Invoice / Supplier..."
              ...
              <select value={suppFilter} onChange={e => setSuppFilter(e.target.value)}
                ...
                {uniqueSuppliers.map(s => <option key={s}>{s}</option>)}
              </select>
            </div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <Th c="Gross (₹)" align="right" /><Th c="Disc (₹)" align="right" />
                  <Th c="Tax (₹)" align="right" /><Th c="Net (₹)" align="right" />
              ...
              {!loading && filteredInvoices.length > 0 && (
                <tfoot>
                  <tr style={{ background: '#f9fafb', borderTop: '2px solid #e5e7eb' }}>
                    ...
                    <td style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 700 }}>{fmt(filteredInvoices.reduce((s, i) => s + (i.gross || 0), 0))}</td>
                    <td style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 700, color: '#16a34a' }}>{fmt(filteredInvoices.reduce((s, i) => s + (i.disc || 0), 0))}</td>
                    <td style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 700 }}>{fmt(filteredInvoices.reduce((s, i) => s + (i.tax || 0), 0))}</td>
                    <td style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 800, color: '#0c3b73' }}>{fmt(filteredInvoices.reduce((s, i) => s + (i.net || 0), 0))}</td>
                    <td colSpan={2} />
                  </tr>
                </tfoot>
              )}
          ...
          <div style={{ padding: '10px 16px', borderTop: '1px solid #f3f4f6' }}>
            <span style={{ fontSize: 11, color: '#6b7280' }}>Showing 1 to {filteredInvoices.length} of {invoices.length} invoices</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr><Th c="#" /><Th c="Medicine Name" /><Th c="Qty" align="center" /><Th c="Amount (₹)" align="right" /></tr></thead>
              <tbody>
                {loading
                  ? Array(5).fill(0).map((_, i) => <tr key={i}><td colSpan={4} style={{ padding: '8px 12px' }}><div style={{ height: 12, background: '#f3f4f6', borderRadius: 4 }} /></td></tr>)
                  : topItems.length > 0
                    ? topItems.map((t, i) => (
                      <tr key={i}
                        onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
                    ...
                    : <tr><td colSpan={4} style={{ padding: '16px', textAlign: 'center', color: '#9ca3af', fontSize: 12 }}>No data for today</td></tr>
                }
              </tbody>
            </table>
          </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
              {['All', 'Cash', 'UPI', 'Credit', 'Paid', 'Unpaid'].map(f => (
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
              {[
                ...
                { label: 'GRN Entry',       icon: Package,      color: '#d97706', bg: '#fef3c7', to: '/franchise/purchase/grn' },
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 0 }}>
          {STATIC_ALERTS.map((a, i) => (
            <div key={i} style={{ padding: '14px 14px', borderRight: i < STATIC_ALERTS.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
                <a.icon size={14} color={a.iconColor} />
                <span style={{ fontSize: 12, fontWeight: 700, color: '#111827' }}>{a.name}</span>
              </div>
              <p style={{ fontSize: 11, color: '#6b7280', margin: '0 0 10px', lineHeight: 1.4, whiteSpace: 'pre-line' }}>{a.msg}</p>
              <button onClick={() => navigate('/franchise/live-rates')}
                style={{ fontSize: 11, fontWeight: 600, padding: '5px 10px', borderRadius: 6, border: `1px solid ${a.btnColor}30`, background: `${a.btnColor}10`, color: a.btnColor, cursor: 'pointer' }}>
                {a.btnLabel}
              </button>
            </div>
          ))}
