...
import { UserCheck, Plus, Eye, Edit2, Search, Trash2, X, Save } from 'lucide-react'
...
const ROLE_COLORS = { 'Franchise Owner': '#0c3b73', 'Branch Manager': '#7c3aed', Pharmacist: '#16a34a', Cashier: '#d97706' }
const ATT_COLORS  = { Present: '#16a34a', Absent: '#dc2626', Late: '#d97706', 'Half Day': '#0891b2' }
...
export default function StaffUsers() {
  ...
  const openEdit = (s) => { setForm({ name: s.name, role: s.role, department: s.department, phone: s.phone, email: s.email || '', shift: s.shift, salary: s.salary }); setEditId(s._id); setModal(true) }
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <PageHeader icon={UserCheck} title="Staff & Users" subtitle="Manage pharmacy staff and their roles" color="#0c3b73">
        <button onClick={openAdd}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#0c3b73', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Total Staff',  value: loading ? '...' : kpi.total,   color: '#0c3b73', bg: '#e0e7ff' },
          { label: 'Present',      value: loading ? '...' : kpi.present, color: '#16a34a', bg: '#dcfce7' },
          { label: 'Absent',       value: loading ? '...' : kpi.absent,  color: '#dc2626', bg: '#fee2e2' },
          { label: 'Late',         value: loading ? '...' : kpi.late,    color: '#d97706', bg: '#fef3c7' },
        ].map(k => (
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '12px 16px', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
          ...
          <input value={search} onChange={e => handleSearchChange(e.target.value)} placeholder="Search staff..."
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              {['ID', 'Name', 'Role', 'Dept', 'Phone', 'Shift', "Today's Sales", 'Attendance', 'Status', 'Actions'].map(h => <Th key={h} c={h} />)}
            </tr></thead>
            <tbody>
              {loading
                ? Array(5).fill(0).map((_, i) => <tr key={i}>{Array(10).fill(0).map((_, j) => <td key={j} style={{ padding: '11px 14px' }}><div style={{ height: 13, background: '#f3f4f6', borderRadius: 4 }} /></td>)}</tr>)
                : staff.length === 0
                  ? <tr><td colSpan={10} style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>No staff found</td></tr>
                  : staff.map(s => (
                    <tr key={s._id}
                      onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
                      onMouseLeave={e => e.currentTarget.style.background = ''}>
                      <Td><span style={{ fontFamily: 'monospace', fontSize: 11, background: '#f3f4f6', padding: '2px 6px', borderRadius: 4 }}>{s.staffId}</span></Td>
                      ...
                      <Td>
                        <div style={{ display: 'flex', gap: 5 }}>
                          <button onClick={() => openEdit(s)} style={{ padding: '4px 8px', border: 'none', borderRadius: 5, background: '#fef3c7', color: '#d97706', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                            <Edit2 size={10} /> Edit
                          </button>
                          <button onClick={() => handleDelete(s._id)} style={{ padding: '4px 7px', border: 'none', borderRadius: 5, background: '#fee2e2', color: '#dc2626', cursor: 'pointer' }}>
                            <Trash2 size={11} />
                          </button>
                        </div>
                      </Td>
