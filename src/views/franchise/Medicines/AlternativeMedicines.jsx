...
/**
 * Screen 22 — Alternative Medicines
 */
import { useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import { RefreshCw, ArrowLeft, Plus, Trash2 } from 'lucide-react'
import PageHeader from '../components/PageHeader'
import { MEDICINES, ALTERNATIVES } from './medicineMockData'
...
export default function AlternativeMedicines() {
  ...
  const removeAlt = i => setAlts(p => p.filter((_, j) => j !== i))

  return (
    <div style={{ fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <PageHeader icon={RefreshCw} title="Alternative Medicines" subtitle="View alternative medicine suggestions for a selected medicine" color="#7c3aed">
      ...
      <div style={{ background: 'linear-gradient(135deg,#7c3aed,#a855f7)', borderRadius: 14, padding: '20px 24px', color: '#fff', display: 'flex', alignItems: 'center', gap: 18 }}>
        ...
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 12, opacity: 0.75 }}>MRP: ₹ {med.mrp.toFixed(2)} · Stock: {med.stock} units</p>
        </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {alts.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>No alternatives added yet</td></tr>
              ) : alts.map((a, i) => (
