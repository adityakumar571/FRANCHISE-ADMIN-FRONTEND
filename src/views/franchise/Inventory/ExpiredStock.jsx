...
export default function ExpiredStock() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true)
      try {
        const res = await getRequest('/franchise/inventory/expired')
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: '#9ca3af', margin: '2px 0 0' }}>Medicines that have crossed expiry date ({items.length} batches)</p>
        </div>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {loading
                ...
                : filtered.length === 0
                  ? <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>{loading ? 'Loading...' : 'No expired stock found'}</td></tr>
                  : filtered.map((r, i) => (
        ...
        <div style={{ padding: '10px 16px', borderTop: '1px solid #f3f4f6' }}>
          <span style={{ fontSize: 12, color: '#6b7280' }}>Total: {filtered.length} expired batches</span>
        </div>
      </div>
    </div>
  )
