...
import { MEDICINES, CUSTOMERS } from './posMockData'
...
export default function NewBilling() {
  ...
  const handleSearch = val => {
    ...
    setSuggest(MEDICINES.filter(m => m.name.toLowerCase().includes(val.toLowerCase()) || m.batch.toLowerCase().includes(val.toLowerCase())).slice(0, 7))
  }

  const addToCart = med => {
    setCart(p => {
      const ex = p.find(i => i.id === med.id)
      return ex ? p.map(i => i.id === med.id ? { ...i, qty: i.qty + (med.qty || 1) } : i) : [...p, { ...med, qty: med.qty || 1 }]
    })
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px' }}>
            <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
              <button onClick={() => navigate('/franchise/pos/customer-selection')}
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input ref={searchRef} value={search} onChange={e => handleSearch(e.target.value)} autoFocus
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ padding: '11px 14px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 700 }}>Cart ({cart.length} medicines · {cart.reduce((s,i)=>s+i.qty,0)} qty)</p>
              {cart.length > 0 && <button onClick={() => setCart([])} style={{ fontSize: 11, color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Clear All</button>}
            </div>
        ...
        {/* RIGHT — Bill Summary */}
        <BillSummaryPanel cart={cart} discount={discount} setDiscount={setDiscount}
          footer={
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <SBtn label="Proceed to Pay [F5]" icon={null} full disabled={cart.length === 0}
                onClick={() => navigate('/franchise/pos/payment', { state: { cart, discount, total } })} />
