...
/**
 * Screen 3 — Medicine Search — Real API integration
 */
...
const CATEGORIES = ['All Categories', 'Analgesic', 'Antibiotic', 'Antidiabetic', 'Cardiac', 'Vitamin', 'GI']
...
export default function MedicineSearch() {
  ...
  const fetchMedicines = useCallback(async (query, category) => {
    setLoading(true)
    setSearched(true)
    try {
      const catParam = category === 'All Categories' ? '' : category
      const res = await getRequest(`/franchise/pos/medicines/search?q=${encodeURIComponent(query)}&category=${encodeURIComponent(catParam)}&limit=30`)
  ...
  const handleSearch = (val) => {
    ...
    debounceRef.current = setTimeout(() => fetchMedicines(val, cat), 400)
  }
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 16px', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
              ...
              <input
                ...
                placeholder="Search medicine name, salt, company..."
                autoFocus
          ...
          <div style={{ padding: '10px 16px', background: '#f9fafb', borderBottom: '1px solid #f3f4f6' }}>
            <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>
              Medicine List — <strong>{medicines.length} Results</strong>
              {!searched && <span style={{ marginLeft: 8, color: '#9ca3af' }}>Start typing to search...</span>}
            </p>
          </div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>{['Medicine Name', 'Salt / Strength', 'Company', 'MRP', 'Stock'].map(h => <Th key={h} c={h} />)}</tr>
              </thead>
              <tbody>
                {loading
                  ...
                  : medicines.map(m => (
                    <tr key={m.id}
                      ...
                      <Td>{m.company}</Td>
                      <Td style={{ fontWeight: 700, color: '#0c3b73' }}>₹ {Number(m.mrp).toFixed(2)}</Td>
                ...
                {!loading && searched && medicines.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ padding: '28px', textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>
          ...
          {sel ? (
            <>
              ...
              {[
                ...
                ['Company',  sel.company],
                ['Pack',     sel.packSize],
                ['MRP',      `₹ ${Number(sel.mrp).toFixed(2)}`],
                ['Stock',    `${sel.stock} strips`],
                ['GST',      `${sel.gst}%`],
                ['Rack',     sel.rackLabel || '—'],
              ].map(([l, v]) => (
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 12px', marginBottom: 12, display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                  ...
                  <span style={{ fontWeight: 800, color: '#0c3b73' }}>₹ {(Number(sel.mrp) * qty).toFixed(2)}</span>
                </div>
                <SBtn label="Add to Bill [F4]" icon={Plus} full onClick={() => navigate('/franchise/pos/billing', { state: { addItem: { ...sel, qty } } })} />
              </div>
            </>
          ) : (
