...
export default function Attendance() {
  ...
  const fetchAttendance = async () => {
    setLoading(true)
    try {
      const res = await getRequest(`franchise/staff/attendance?date=${date}`)
      setData(res?.data || [])
    } catch {
      setData([])
    } finally {
  ...
  const late    = data.filter(m => m.status === 'Late').length
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 12 }}>
        {[
          { label: 'Total Staff', value: data.length, color: '#0c3b73', icon: Users },
      ...
      {data.length > 0 && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px' }}>
          ...
          <div style={{ height: 10, borderRadius: 5, background: '#f3f4f6', overflow: 'hidden', display: 'flex' }}>
            <div style={{ width: `${(present / data.length) * 100}%`, background: '#16a34a' }} />
            <div style={{ width: `${(late / data.length) * 100}%`, background: '#d97706' }} />
            <div style={{ width: `${(absent / data.length) * 100}%`, background: '#dc2626' }} />
            <div style={{ width: `${(leave / data.length) * 100}%`, background: '#4f46e5' }} />
          </div>
          <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
            {[['#16a34a','Present',present],['#d97706','Late',late],['#dc2626','Absent',absent],['#4f46e5','Leave',leave]].map(([c,l,v]) => (
              <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <span style={{ fontSize: 11, color: '#6b7280' }}>{l} ({data.length ? ((v/data.length)*100).toFixed(0) : 0}%)</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={m._id || m.staffId} onMouseEnter={e => e.currentTarget.style.background='#fafafa'} onMouseLeave={e => e.currentTarget.style.background=''}>
                <Td><span style={{ fontFamily: 'monospace', fontSize: 11, background: '#f3f4f6', padding: '2px 7px', borderRadius: 4 }}>{m.staffId || m._id?.slice(-6)}</span></Td>
                <Td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#0c3b7322', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#0c3b73', flexShrink: 0 }}>{(m.name||'S')[0]}</div>
        ...
        <div style={{ padding: '10px 16px', borderTop: '1px solid #f3f4f6' }}>
          <span style={{ fontSize: 12, color: '#6b7280' }}>Showing {filtered.length} of {data.length} staff members</span>
        </div>
      </div>
    </div>
  )
