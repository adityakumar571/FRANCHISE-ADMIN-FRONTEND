...
import { Landmark, Download, Search, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function BankBook() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await getRequest(`/franchise/accounts/bank-book?from=${from}&to=${to}&page=${page}&limit=${LIMIT}&search=${encodeURIComponent(search)}`)
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {loading
                ...
                : entries.map((e,i)=>(
                  <tr key={e._id||i} style={{ background:i%2===0?'#fff':'#fafafa' }}>
