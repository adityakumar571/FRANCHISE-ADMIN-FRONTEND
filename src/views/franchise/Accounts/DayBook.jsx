...
export default function DayBook() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await getRequest(
        `/franchise/accounts/day-book?from=${from}&to=${to}&page=${page}&limit=${LIMIT}&search=${encodeURIComponent(search)}&type=${typeFilter}`
      )
  ...
  }, [from, to, page, search, typeFilter])

  useEffect(() => { fetchData() }, [fetchData])

  const handleSearch = (val) => { setSearch(val); setPage(1) }
  const handleType   = (val) => { setTypeFilter(val); setPage(1) }
  ...
  return (
    <div style={{ fontFamily:'Inter, sans-serif', display:'flex', flexDirection:'column', gap:18 }}>
      ...
      <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:10, padding:'12px 16px', display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
        ...
        {(from||to||search||typeFilter) && <button onClick={() => { setFrom(''); setTo(''); setSearch(''); setTypeFilter(''); setPage(1) }} style={{ padding:'7px 14px', border:'1px solid #e5e7eb', borderRadius:7, fontSize:12, cursor:'pointer' }}>Clear</button>}
      </div>
                    ...
                    <tr key={e._id||i} onMouseEnter={x=>x.currentTarget.style.background='#fafafa'} onMouseLeave={x=>x.currentTarget.style.background=''}>
                      ...
                      <Td>
                        <span style={{ fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:20,
                          background:e.type==='Sale'?'#dcfce7':e.type==='Purchase'?'#fee2e2':'#f3f4f6',
                          color:e.type==='Sale'?'#16a34a':e.type==='Purchase'?'#dc2626':'#6b7280' }}>
                          {e.type||'—'}
                        </span>
                      </Td>
